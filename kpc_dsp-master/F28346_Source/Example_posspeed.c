...
// Checkin $Date: July 31, 2009   09:55:56 $
//###########################################################################
//
...
// functions called by Example_2834xEqep_posspeed.c.  The position and
...
// If max/base speed = 6000 rpm:  6000 = (16/4000)/[(t2-t1)/(300MHz/128)]
//    where 16 edges = QCAPCTL [UPPS] (Unit timeout - once every 16 edges)
//          16/4000 = position in 1 revolution (position as a fraction of 1 revolution)
//          t2-t1/(300MHz/128),  t2-t1= # of QCAPCLK cycles, and
//		                  1 QCAPCLK cycle = 1/(300MHz/128)
//										  = QCPRDLAT
//
//		        So: 6000 rpm = [16(300MHz/128)*60s/min]/[4000(t2-t1)]
//		             t2-t1 = [16(300MHz/128)*60 s/min]/(4000*6000rpm)           - Equation 5
//		                   = 94 CAPCLK cycles = maximum (t2-t1) = SpeedScaler
//
// Divide both sides by (t2-t1), and:
//                   1 = 94/(t2-t1) = [16(300MHz/128)*60 s/min]/(4000*6000rpm)]/(t2-t1)
...
//
//					 speed_pr = 94/(t2-t1)
//                      or [16(300MHz/128)*60 s/min]/(4000*6000rpm)]/(t2-t1)  - Equation 6
//
// To convert speed_pr to RPM:
// Multiply Equation 6 by 6000rpm:
//                  SpeedRpm_fr  = 6000rpm * [16(300MHz/128)*60 s/min]/[4000*6000rpm*(t2-t1)]
//							                = [16(300MHz/128)*60 s/min]/[4000*(t2-t1)]
//                                        or [(16/4000)rev * 60 s/min]/[(t2-t1)(QCPRDLAT)]- Final Equation
//
//
// For 250/200 MHz Operation:
// --------------------------
//
// The same calculations as above are performed, but with 250/200 MHz
// instead of 300MHz when calculating SpeedRpm_pr.
// For 250 MHz operation:
// The value for freqScaler_pr becomes: [16*(250MHz/128)*60s/min]/(4000*6000rpm) = 78
// For 200 MHz operation:
// The value for freqScaler_pr becomes: [16*(200MHz/128)*60s/min]/(4000*6000rpm) = 63
...
#include "..\F28346_Include\Example_posspeed.h"   // Example specific Include file

// smartec code ------------------------
#define ENCODER_PULS_4_MULT		40000-1
// -------------------------------------
...
void  POSSPEED_Init(void)
{
    ...
    #if (CPU_FRQ_300MHZ)
	//for High Speed 
	  EQep1Regs.QUPRD=3000000;			// Unit Timer for 100Hz at 300 MHz SYSCLKOUT
	#endif
	
    #if (CPU_FRQ_250MHZ)
    //for High Speed
	  EQep1Regs.QUPRD=2500000;			// Unit Timer for 100Hz at 200 MHz SYSCLKOUT
	#endif
	...
	EQep2Regs.QEPCTL.bit.PCRM=00;		// PCRM=00 mode - QPOSCNT reset on First Z pulse 
	EQep2Regs.QEPCTL.bit.UTE=1; 		// Unit Timeout Enable
	EQep2Regs.QEPCTL.bit.QCLM=1; 		// Latch on unit time out
	EQep2Regs.QEINT.bit.IEL = 1;	//intterupt
	//EQep1Regs.QPOSMAX=0xffffffff;							// original code 
	EQep2Regs.QPOSMAX = (ENCODER_PULS_4_MULT);			// dsbaek code 
	EQep2Regs.QEPCTL.bit.QPEN=1; 		// QEP enable

	EQep2Regs.QCAPCTL.bit.UPPS=4;   	// 1/16 for unit position	for Low Speed measurement
	EQep2Regs.QCAPCTL.bit.CCPS=7;		// 1/128 for CAP clock		for Low Speed	measurement
	EQep2Regs.QCAPCTL.bit.CEN=1; 		// QEP Capture Enable			for Low Speed	measurement
...
{
     unsigned long nTemp_Val1;	
     unsigned int pos16bval;

//**** Position calculation - mechanical and electrical motor angle  ****//
     p->DirectionQep = EQep2Regs.QEPSTS.bit.QDF;    // Motor direction: 0=CCW/reverse, 1=CW/forward

	 nTemp_Val1 = EQep2Regs.QPOSCNT;
	 pos16bval=(unsigned int)nTemp_Val1;     // capture position once per QA/QB period
     p->theta_raw = pos16bval + p->cal_angle;       // raw theta = current pos. + ang. offset from QA




// smartec code (Calc System angle)--------------------------------------
	 // The following lines calculate p->theta_mech ~= QPOSCNT/mech_scaler [current cnt/(total cnt in 1 rev.)]
	 // where mech_scaler = 40000 cnts/revolution
	 // 1 revolute [rev],	precision 1/40000

// add dskim
//     nTemp_Val2 = EQep1Regs.QPOSCNT;

	 if(p->theta_raw < 0)
		 p->theta_raw = p->theta_raw + (ENCODER_PULS_4_MULT);
	 else if(p->theta_raw > (ENCODER_PULS_4_MULT))
		 p->theta_raw = p->theta_raw - (ENCODER_PULS_4_MULT);


/*	 p->theta_mech = (long)((long)p->theta_raw * (long)p->mech_scaler);  
	 p->CurTheta = p->theta_mech;
	 p->Dtheta = p->CurTheta - p->PreTheta;

	if(p->Dtheta > _IQ(0.5)) 
		p->Mot_Rev = p->Mot_Rev - 1;  // CW  감소 방향 
	if(p->Dtheta < _IQ(-0.5)) 
		p->Mot_Rev = p->Mot_Rev + 1; // CCW  증가 방향 
*/
	 p->theta_mech = (long)((long)p->theta_raw);	// * (long)p->mech_scaler);  
	 p->CurTheta = p->theta_mech;
	 p->Dtheta = p->CurTheta - p->PreTheta;
...
//    	p->index_sync_flag = 0x00F0;
  ...
  //  	EQep1Regs.QCLR.bit.IEL=1;					// Clear interrupt flag
	...
	p->Sys_Angle = p->Mot_Rev * 40000  + p->CurTheta;	//p->CurTheta;
	p->PreTheta = p->CurTheta;
//----------------------------------------------------------------------
